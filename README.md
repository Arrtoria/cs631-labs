...
因为这门课要在Linux系统下进行。仅针对此次作业，我用的是ubuntu 22.04。

本文你可以看成是一个基础内容大杂烩，内容并不单单涉及作业，而是一个笼统的介绍和简单的代码展示，作业只是作为一个展示的大纲，额外会有很多很有用又很简单的补充内容（仅针对初学者）。
## 收获
...
- 能够编写简单的Unix系统程序，并理解其运行原理和机制（该作业实现了一个仅针对文件的cp）。
...
新手学的C语言在很大程度上像一个封闭的圈，除了能在那个黑框框里输出不同的东西什么也做不了。而本次作业会逐渐解答这样的问题。我们并不需要花很大的精力就可以突破这个圈。
...
不过话又说回来了，既然我们知道`argc = 3`, 那我们是不是要处理argc不为3的情况？真实生活中你的客户并不总是按照你的预期输入，就像你告诉他你写了一个判断数的大小的程序而他要输入a和b一样。如果你不做任何处理，你的程序就会把他当成整数来处理，会有什么后果呢，咱也不知道。（当然如果你对ASCII码比较了解的话你会知道这个举例中a < b,但有的不合法的输入可能导致更严重的后果，比如程序终止。甚至黑客们会利用这一点操控你的机器，偷窃甚至修改你的数据）所以我们要把这种可能扼杀在摇篮里，禁止不符合预期的输入。
所以我们要这样做
```C
if (argc != 3) {
